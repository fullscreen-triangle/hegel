...
\begin{abstract}
We report direct observation of electron trajectories during carbonic anhydrase~II (CA~II) catalysis using a zero-backaction measurement protocol. Traditional enzyme kinetics interprets CA~II's exceptional turnover number ($\kcat \approx 10^6$~s$^{-1}$) as evidence of ``near-diffusion-limited temporal acceleration'' through transition state stabilization. We demonstrate instead that catalysis proceeds through a \emph{single categorical transition} ($\dC = 1$) in phase-lock network space---the enzyme provides a short topological pathway through configuration space, not temporal acceleration along a fixed pathway. Our measurements achieve $8{,}717\times$ reduction in momentum disturbance compared to Heisenberg-limited detection, enabling trajectory reconstruction with 76~pm spatial resolution over 100~ps timescales. The complete catalytic cycle (\ce{CO2} $\to$ \ce{HCO3-}) is observed as geometric aperture traversal through the \ce{Zn^{2+}} coordination sphere. Phase coherence remains near-unity ($\langle R \rangle > 0.999$) throughout the catalytic cycle, confirming deterministic trajectories in categorical space despite thermal fluctuations in physical space. We derive the relationship $\kcat \propto 1/(\dC \cdot \tau_{\text{step}})$, where $\tau_{\text{step}}$ is the intrinsic time for one categorical transition, explaining why CA~II's minimal $\dC = 1$ corresponds to maximal turnover. The framework resolves three fundamental paradoxes of enzyme catalysis: the instantaneous concentration paradox, the reversible reaction paradox, and the step-exclusion paradox. We conclude that enzymes are categorical apertures---geometric constraints in phase-lock network space that select molecules by configurational complementarity, not kinetic properties---and that turnover rate reflects topological distance rather than activation energy.
\end{abstract}
