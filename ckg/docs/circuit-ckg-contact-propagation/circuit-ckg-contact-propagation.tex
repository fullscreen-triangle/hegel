...
probes separates the pair the other handles. Hence three probes with mutually
non-substitutable observables are necessary and, on that system, sufficient
\citep{Denisov2005,Rittle2010}.

\begin{corollary}[Panel floor]
\label{cor:three}
For a state space whose collisions have the structure above, a probe panel must
contain at least three members with pairwise non-substitutable observables.
Redundant copies of one probe do not raise discrimination.
\end{corollary}
...
observables whose \emph{disjointness} is what buys resolution. A panel of three
identical reasoners has the discriminating power of one.
\end{remark}
...
\item \textbf{Coordinate irreducibility.} No axis of $\Scoord$ is a function of
the other two on the test networks; triple coherence $\mathcal{R}$ is reported
...
asymptotics.
\item \textbf{Panel disjointness.} Every pair of reasoners carries a
certificate per \cref{def:disjointness}, \emph{including} the isomorphism check.
